...
/* #endif */
{
    ...
    /**
     * Return Java-level implementation type. The type used to implement types
     * not natively understood by the JVM or the Java language. Usually, the
     * identity function. However, a language might handle union types or
     * template types or type expressions calculated at run time. In that case
     * return the type used at the Java level, and known at compile time.
     */
    ...
    static java.util.HashMap<String, ClassType> mapNameToClassType;
    ...
    /**
     * Find an Type with the given name, or create a new one. Use this for
    ...
    /**
     * Try to map java.lang.reflect.Type to gnu.bytecode.Type. If we can't
     * handle that, resolve the Class instead.
     */
    ...
    /**
     * Resolve a java.lang.reflect.Type to gnu.bytecode.Type. Handles simple
     * parameterized types, but does not handle type variables.
     * 
    ...
    static Type make(java.lang.reflect.Type type)
    {
        ...
        if (type instanceof java.lang.reflect.GenericArrayType)
        {
            if (type instanceof java.lang.reflect.GenericArrayType)
            {
                java.lang.reflect.GenericArrayType gatype = (java.lang.reflect.GenericArrayType) type;
        ...
        if (type instanceof java.lang.reflect.ParameterizedType)
        {
            ...
            if (rt instanceof ClassType)
            {
                ...
                for (int i = 0; i < nargs; i++)
                {
                    ...
                    {
                        java.lang.reflect.WildcardType wi = (java.lang.reflect.WildcardType) ti;
    ...
    public static void printSignature(String sig, int off, int len, java.io.PrintWriter out)
    {
        if (len == 0)
            return;
        char c = sig.charAt(off);
    ...
    /**
     * Returns the Java-level type name from a given signature. Returns null for
     ...
     * 
     * @return -1 if not compatible; 0 if need to check at run-time; 1 if
     *         compatible; 2 if compatible and no conversion or cast needed. We
     *         also return 0 for some "narrowing" conversions even if we know
     *         they will always succeed, so as to make such conversions less
     *         preferred when doing method overloading.
     */
     ...
     *
     * Interfaces are not taken into account. This would be difficult, since
     * interfaces allow multiple-inheritance. This means that there may exists
     * multiple common supertypes to t1 and t2 that are not comparable.
     *
     * @return the lowest type that is both above t1 and t2, or null if t1 and
     *         t2 have no common supertype.
     */
    ...
    /**
     * Return a numeric code showing "subtype" relationship: 1: if other is a
     * pure subtype of this; 0: if has the same values; -1: if this is a pure
     * subtype of other; -2: if they have values in common but neither is a
     * subtype of the other; -3: if the types have no values in common. "Same
     * values" is rather loose; by "A is a subtype of B" we mean that all
     * instance of A can be "widened" to B. More formally, A.compare(B) returns:
     * 1: all B values can be converted to A without a coercion failure (i.e. a
     * ClassCastException or overflow or major loss of information), but not
     * vice versa. 0: all A values can be converted to B without a coercion
     * failure and vice versa; -1: all A values can be converted to B without a
     * coercion failure but not vice versa; -2: there are (potentially) some A
     * values that can be converted to B, and some B values can be converted to
     * A; -3: there are no A values that can be converted to B, and neither are
     * there any B values that can be converted to A.
     */
    ...
    /**
     * Convert an object to a value of this Type. The result is actually of the
     * implementation type, boxed as appropriate, so it is suitable for standard
     * reflective operations, like the arguments to Field#set or Method#invoke.
     * Throw a ClassCastException when this is not possible.
     */
    public abstract Object coerceFromObject(Object obj);

    /**
     * Given a raw JVM value convert it to an object of this type. I.e. the
     * argument is an object of the type returned by {@code getRawType()}, boxed
     * as needed. The result may be a language-specific (boxed) value. Generally
     * a no-op.
     */
    ...
    /**
     * Convert from stackType (usually PrimType) to this type. However, we might
     * only convert part-way, to some object type. If converting to this type
     * might fail at run-time, only convert to Object (as by
     * emitCoerceToObject); a caller can use
     * {@code stackType.emitConvertFromObject} to convert the rest, but that
     * might throw an exception. (This is a bit of a kludge.)
     */
    ...
    public static final PrimType byteType = new PrimType("byte", "B", 1, java.lang.Byte.TYPE);
    public static final PrimType shortType = new PrimType("short", "S", 2, java.lang.Short.TYPE);
    public static final PrimType intType = new PrimType("int", "I", 4, java.lang.Integer.TYPE);
    public static final PrimType longType = new PrimType("long", "J", 8, java.lang.Long.TYPE);
    public static final PrimType floatType = new PrimType("float", "F", 4, java.lang.Float.TYPE);
    public static final PrimType doubleType = new PrimType("double", "D", 8, java.lang.Double.TYPE);
    public static final PrimType booleanType = new PrimType("boolean", "Z", 1,
        java.lang.Boolean.TYPE);
    public static final PrimType charType = new PrimType("char", "C", 2, java.lang.Character.TYPE);
    public static final PrimType voidType = new PrimType("void", "V", 0, java.lang.Void.TYPE);
    ...
    static
    {
        mapNameToClassType = new java.util.HashMap<String, ClassType>();
        /* #ifdef JAVA5 */
    ...
    public static final ClassType javalangannotationAnnotationType = ClassType
        .make("java.lang.annotation.Annotation");
    ...
    public static final Method intValue_method = javalangNumberType.addMethod("intValue",
        typeArray0, intType, Access.PUBLIC);
    public static final Method longValue_method = javalangNumberType.addMethod("longValue",
        typeArray0, longType, Access.PUBLIC);
    public static final Method floatValue_method = javalangNumberType.addMethod("floatValue",
        typeArray0, floatType, Access.PUBLIC);
    public static final Method doubleValue_method = javalangNumberType.addMethod("doubleValue",
        typeArray0, doubleType, Access.PUBLIC);
    public static final Method booleanValue_method = javalangBooleanType.addMethod("booleanValue",
        typeArray0, booleanType, Access.PUBLIC);
    public static final ClassType javalangClassType = ClassType.make("java.lang.Class");

    public static final ClassType javalanginvokeMethodHandleType = ClassType
        .make("java.lang.invoke.MethodHandle");
    ...
    public static final ObjectType errorType = new SpecialObjectType("<error-type>",
        javalangObjectType);
    ...
    public static final ObjectType toStringType = new SpecialObjectType("String",
        javalangStringType);
