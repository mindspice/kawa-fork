...
public class Label
{
    ...
    org.objectweb.asm.Label asmLabel;

    /**
     * The PC of where the label is, or -1 if not yet defined. The value -2
     * means don't generate a StackMapTable entry. This PC may be tentative if
     * we later run processFixups. The offset in the code array is
     * cattr.fixupOffset(first_fixup).
     */
    int position;
    ...
    public void setTypes(CodeAttr code)
    {
        ...
        if (stackTypes != null && code.SP != stackTypes.length)
            throw new InternalError("bad type SP:" + code.SP + " st.l:" + stackTypes.length
                + " code:" + code.getMethod());
        setTypes(code.local_types, code.local_types == null ? 0 : code.local_types.length,
            code.stack_types, code.SP);
    }

    public void setTypes(Label other)
    {
        setTypes(other.localTypes, other.localTypes.length, other.stackTypes,
            other.stackTypes.length);
    }
    ...
    /**
     * Map from Variable number to set of listeners. When
     * {@code this.localTypes[varnum]} is invalidated, then we also need to
     * invalidate that variable in all the listeners. The type is actually a
     * {@code Union<Label,ArrayList<Label>>[]}.
     */
    ...
    void addTypeChangeListeners(CodeAttr code)
    {
        if (code.local_types != null && code.previousLabel != null)
        {
            ...
            {
                if (code.local_types[varnum] != null && (code.varsSetInCurrentBlock == null
    ...
    /*
     * DEBUG int id = ++counter; static int counter; public String toString() {
     * return "Label#"+id+"-pos:"+position; }
     */
}
