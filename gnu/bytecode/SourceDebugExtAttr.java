...
/**
 * Represents the contents of a JSR-45 "SourceDebugExtension" attribute. We only
 * support generating a single "stratum".
 */
...
public class SourceDebugExtAttr extends Attribute
{
    ...
    /**
     * Number of files in the <code>FileSection</code>. The active (used) length
     * of the <code>fileNames</code> and <code>fileIDs</code> arrays.
     */
    int fileCount;
    /**
     * For each entry in the <code>FileSection</code>:
     * <code>2*id+with_path</code>. The <code>id</code> is the
     * <code>FileID</code> written in the SMAP; the <code>with_path</code> is 1
     * if we also write the full path.
     */
    int[] fileIDs;
    /**
     * For eachentry its <code>FileName</code>. If the <code>with_path</code>
     * bit is set, <code>FileName '\n' AbsoluteFileName</code>.
     */
    String[] fileNames;
    ...
    private int fixLine(int sourceLine, int index)
    {
        ...
        else if (index == 5 * (lineCount - 1) || (index == 0 && sourceLine < lines[5 + 3]))
    ...
    public String getData()
    {
        ...
        // Append SMAP Header:
        sbuf.append("SMAP\n");
        nonAsteriskString(outputFileName, sbuf);
        sbuf.append('\n');
