...
public class Variable extends Location implements java.util.Enumeration
{
    ...
    public static final int LIVE_FLAG = 0x4;

    static final int UNASSIGNED = -1;
    /**
     * The local variable slot number used by this variable. Not used (by the
     * codegen layer) if !isSimple().
     */
    public/**/ int offset = UNASSIGNED;

    /**
     * Returns true iff assigned to a local variable slot. Only relevant if
     * isSimple ().
     */
    public final boolean isAssigned()
    {
        return offset != UNASSIGNED;
    }
    ...
    /**
     * Returns true for a "simple" variable. A "simple" Variable can be
     * stack-allocated using standard local variable slots. It is allocated by
     * the codegen package. A non-simple variable may need heap allocation, or
     * more more complex access; it is basically ignored by the codegen package,
     ...
     * 
     * @param varIndex
     *            the index of the local variables.
    ...
    {
        if (getType().getRawType() instanceof ClassType)
            code.getMethod().classfile.addToTypeMap((ClassType) getType().getRawType());
    ...
    boolean shouldEmit()
    {
        ...
        Label start, end;
        int pos;
        return (isSimple() && name != null && sc != null && (start = sc.start) != null
            && (pos = start.position) >= 0 && (end = sc.end) != null && end.position > pos);
    }
